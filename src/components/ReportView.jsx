...
// Corpo visual do relatório executivo, compartilhado entre a visualização
// interna (RelatorioVisualizar) e o link público (PublicRelatorio). Só desenha
// o conteúdo (capa -> anexos); as barras de ação ficam em cada página que o usa.
    ...
    <>
          ...
          <div className="flex flex-col gap-3 mb-9">
            {[
              ...
              <div key={tipo} className="flex items-center gap-4">
                <span className="text-[12.5px] text-[#1C2430] w-[160px] md:w-[220px] shrink-0">{label}</span>
      ...
      {movimentacoesOrdenadas.length > 0 && (
        <>
          ...
          <div className="bg-white border border-[#E2E5EA] rounded-lg overflow-x-auto mb-6">
      ...
      {attachmentsCount > 0 && (
        <div className="flex items-center gap-1.5 text-[12px] text-[#9AA2AF] italic mb-6">
          <Paperclip size={12} /> {attachmentsCount} documento(s) de apoio anexado(s) e referenciado(s) nesta análise.
        </div>
      )}

      {status === "rascunho" && (
        <div className="flex items-center gap-2 bg-[#F4EEDD] text-[#8A6D1F] text-[13px] px-4 py-3 rounded-lg mb-6">
          <FileText size={14} /> Este relatório ainda é um rascunho. Continue o assistente para gerar a versão final.
        </div>
      )}
    </>
  );
