...
/**
 * Remove arquivos do Storage a partir das suas keys. É best-effort: falhas são
 * logadas mas não interrompem o fluxo (o registro no banco já foi apagado; um
 * objeto órfão no Storage é problema menor do que quebrar a exclusão).
 */
export async function removeFiles(keys) {
  ...
  try {
    ...
    if (error) console.error("Falha ao remover arquivos do Storage (ignorado):", error.message);
  } catch (err) {
    console.error("Falha ao remover arquivos do Storage (ignorado):", err?.message);
  }
}

/**
 * Baixa um arquivo do Storage como Buffer (ex.: o PDF já gerado, para anexar
 * num e-mail). Lança se o objeto não existir.
 */
