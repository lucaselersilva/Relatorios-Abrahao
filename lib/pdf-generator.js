...
// Cor da faixa/fundo/rótulo do cartão de ponto de atenção por severidade (PDF).
const SEVERIDADE_PDF = {
  alta: { faixa: C.red, fundo: "#FBEAEA", rotulo: "SEVERIDADE ALTA" },
  media: { faixa: C.gold, fundo: "#F7F1E4", rotulo: "SEVERIDADE MÉDIA" },
  baixa: { faixa: C.slate, fundo: "#F1F3F6", rotulo: "SEVERIDADE BAIXA" },
};
...
export async function generateReportPdf({
  ...
  const { sumarioExecutivo, destaques: destaquesLista, pontosDeAtencao } = normalizeNarrativas(narrativas);
  ...
  const sumarioBlock = () => {
    ...
    const h = doc.heightOfString(sumarioExecutivo, { width: CONTENT_W });
    ensureSpace(h + 6);
    doc.fillColor("#2A3140").font("Times-Roman").fontSize(10.5).text(sumarioExecutivo, LEFT, y, { width: CONTENT_W, align: "left" });
    y += h + 14;
  };
  ...
  // -------------------------------------------------------------------------
  // Pontos de atenção (IA) — cartões coloridos por severidade
  // -------------------------------------------------------------------------
  const pontosAtencao = () => {
    heading("Pontos de atenção");
    const padX = 14;
    const padY = 12;
    const innerX = LEFT + padX + 6;
    const innerW = CONTENT_W - padX * 2 - 6;
    pontosDeAtencao.forEach((p) => {
      const sev = SEVERIDADE_PDF[severidadeValida(p.severidade)];
      doc.font("Helvetica-Bold").fontSize(10);
      const tituloH = doc.heightOfString(p.titulo ?? "", { width: innerW });
      doc.font("Helvetica").fontSize(9);
      const textoH = doc.heightOfString(p.texto ?? "", { width: innerW });
      const cardH = padY + 9 + 4 + tituloH + 5 + textoH + padY;
      ensureSpace(cardH + 8);
      const yTop = y;
      doc.save().rect(LEFT, yTop, CONTENT_W, cardH).fill(sev.fundo).restore();
      doc.save().rect(LEFT, yTop, 4, cardH).fill(sev.faixa).restore();
      let ty = yTop + padY;
      doc.fillColor(sev.faixa).font("Helvetica-Bold").fontSize(6.8).text(sev.rotulo, innerX, ty, { width: innerW, characterSpacing: 0.6, lineBreak: false });
      ty += 9 + 4;
      doc.fillColor(C.navy).font("Helvetica-Bold").fontSize(10).text(p.titulo ?? "", innerX, ty, { width: innerW });
      ty += tituloH + 5;
      doc.fillColor("#2A3140").font("Helvetica").fontSize(9).text(p.texto ?? "", innerX, ty, { width: innerW });
      y = yTop + cardH + 10;
    });
  };
  ...
  doc.addPage();
  coverBand();
  if (sumarioExecutivo?.trim()) sumarioBlock();
  kpiCards();
  if (panoramaView.legenda.length > 0) panoramaSection();
  if (movs.length > 0) movBars();
  if (destaquesLista.length > 0) destaques();
  if (pontosDeAtencao.length > 0) pontosAtencao();
