...
export function computeDiff(processosAtuais, processosAnteriores) {
  ...
  const atuaisPorNumero = new Set(processosAtuais.map((p) => p.numero));
  const movimentacoes = [];
  let idSeq = 1;
  ...
  // Caso inverso: processo que existia no mês anterior e sumiu da planilha atual
  // — provável encerramento/baixa. É informação juridicamente relevante que, sem
  // este bloco, simplesmente desaparecia do relatório.
  for (const anterior of processosAnteriores) {
    if (atuaisPorNumero.has(anterior.numero)) continue;
    const valorAnterior = toNumber(anterior.valor);
    movimentacoes.push({
      id: idSeq++,
      tipo: "encerrado",
      numero: anterior.numero,
      parte: anterior.parte,
      area: anterior.area,
      // Não há valor no mês atual; exibimos a última exposição conhecida.
      valor: valorAnterior,
      valorFormatado: formatMoeda(valorAnterior),
      deltaValor: null,
      statusAnterior: anterior.status,
      statusAtual: null,
      resumo: "Processo não consta mais na carteira — provável encerramento/baixa.",
      // Mesmo critério de "novo": exposição alta que saiu da carteira merece
      // destaque de prioridade alta.
      prioridade: valorAnterior != null && valorAnterior >= 1_000_000 ? "alta" : "media",
    });
  }
