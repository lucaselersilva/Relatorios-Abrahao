...
import { normalizeNarrativas, severidadeValida } from "./narrativas.js";
...
const TIPO_INFO = {
  ...
  encerrado: { label: "Encerramentos / baixas", color: SLATE },
};
...
function movimentacoesPorTipoSection(movimentacoes) {
  ...
  const tipos = Object.entries(TIPO_INFO);
  const rows = tipos.map(([tipo, info], i) => {
    const isLast = i === tipos.length - 1;
    const rowBorder = { ...NO_BORDERS, bottom: isLast ? NO_BORDER : { style: BorderStyle.SINGLE, size: 2, color: LINE } };
    return new TableRow({
      children: [
        new TableCell({
...
// ---------------------------------------------------------------------------
// Sumário executivo (parágrafo de abertura em prosa)
// ---------------------------------------------------------------------------

function sumarioExecutivoBlock(texto) {
  return new Paragraph({
    spacing: { after: 60 },
    children: [new TextRun({ text: texto, size: 19, color: "2A3140", font: FONT_SERIF })],
  });
}

// ---------------------------------------------------------------------------
// Pontos de atenção (cartões coloridos por severidade)
// ---------------------------------------------------------------------------

// Cor da faixa/fundo/texto por severidade, no mesmo vocabulário visual dos KPIs.
const SEVERIDADE_DOCX = {
  alta: { faixa: RED, fundo: RED_SOFT, texto: RED, rotulo: "SEVERIDADE ALTA" },
  media: { faixa: GOLD, fundo: GOLD_SOFT, texto: GOLD_TEXT, rotulo: "SEVERIDADE MÉDIA" },
  baixa: { faixa: SLATE, fundo: "EEF0F3", texto: NAVY_LIGHT, rotulo: "SEVERIDADE BAIXA" },
};

function pontoAtencaoCard(p) {
  const sev = SEVERIDADE_DOCX[severidadeValida(p.severidade)];
  return new Table({
    width: { size: 100, type: WidthType.PERCENTAGE },
    rows: [
      new TableRow({
        children: [
          new TableCell({
            shading: { fill: sev.fundo },
            borders: { ...NO_BORDERS, left: { style: BorderStyle.SINGLE, size: 18, color: sev.faixa } },
            margins: { top: 180, bottom: 180, left: 220, right: 260 },
            children: [
              new Paragraph({
                children: [new TextRun({ text: sev.rotulo, bold: true, size: 13, color: sev.texto, font: FONT_SANS, characterSpacing: 10 })],
              }),
              new Paragraph({
                spacing: { before: 40 },
                children: [new TextRun({ text: p.titulo, bold: true, size: 20, color: NAVY, font: FONT_SANS })],
              }),
              new Paragraph({
                spacing: { before: 90 },
                children: [new TextRun({ text: p.texto, size: 18, color: "2A3140", font: FONT_SANS })],
              }),
            ],
          }),
        ],
...
function badgeCell(tipo) {
  const map = {
    ...
    encerrado: { label: "ENCERRADO", fill: "EEF0F3", color: NAVY_LIGHT },
  };
  return map[tipo] ?? map.movimentacao;
}

function variacaoRun(m) {
  if (m.tipo === "encerrado") return new TextRun({ text: "saiu da carteira", size: 15, color: SLATE, italics: true, font: FONT_SANS });
    ...
    return new TableRow({
      children: [
        ...
        new TableCell({
          ...
          children: [new Paragraph({ children: [new TextRun({ text: m.tipo === "encerrado" ? (m.statusAnterior ?? "Encerrado") : (m.statusAtual ?? "-"), size: 15, color: "44546A", font: FONT_SANS })] })],
        }),
      ],
    });
  });
    ...
    children: [
      new Paragraph({
        ...
        children: [
          ...
          new TextRun({ children: ["   —   Página ", new PageNumberElement()], size: 13, color: SLATE, font: FONT_SANS }),
        ],
      }),
    ],
  });
...
export async function generateReportDocx({
  ...
  const { sumarioExecutivo, destaques, pontosDeAtencao } = normalizeNarrativas(narrativas);
  const panoramaSeguro = panorama ?? { porArea: [], topExposicoes: [] };

  const children = [coverBand({ cliente, mesReferencia, versao, geradoEm: geradoEmDate }), emptyPara(120)];

  if (sumarioExecutivo?.trim()) {
    children.push(heading("Sumário executivo"));
    children.push(sumarioExecutivoBlock(sumarioExecutivo));
    children.push(emptyPara(80));
  }

  children.push(kpiCardsRow(kpis, kpisAnterior));
  ...
  if (destaques.length > 0) {
    ...
    destaques.forEach((n, i) => {
      ...
      if (i < destaques.length - 1) children.push(emptyPara(140));
    });
  }

  if (pontosDeAtencao.length > 0) {
    children.push(heading("Pontos de atenção"));
    pontosDeAtencao.forEach((p, i) => {
      children.push(pontoAtencaoCard(p));
      if (i < pontosDeAtencao.length - 1) children.push(emptyPara(140));
    });
  }
